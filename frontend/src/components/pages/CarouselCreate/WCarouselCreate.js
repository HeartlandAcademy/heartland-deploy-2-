...
import { createWCarousel } from "../../../actions/carouselActions";
import { WCAROUSEL_CREATE_RESET } from "../../../actions/types";
...
const AdminCarousel = ({ history }) => {
  ...
  const carouselWCreate = useSelector((state) => state.carouselWCreate);
  const { success, error } = carouselWCreate;
  ...
  useEffect(() => {
    if (success) {
      clearForm();
      dispatch({ type: WCAROUSEL_CREATE_RESET });
      setValidated(false);
  ...
  const carouselHandler = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      dispatch(createWCarousel(title, description, carouselImage));
    }
    setValidated(true);
  };
