...
import { deleteWEvents, listWEvents } from "../../../actions/eventsActions";
...
const AdminAllWEvents = ({ history }) => {
  ...
  const { loading, wEvents: events, error } = upcomingWEvents;
  ...
  const wEventsDelete = useSelector((state) => state.wEventsDelete);
  const { success: deleteSuccess } = wEventsDelete;
...
export default AdminAllWEvents;
