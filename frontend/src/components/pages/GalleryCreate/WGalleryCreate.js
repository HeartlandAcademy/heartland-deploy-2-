...
import { addWGalleryAlbums } from "../../../actions/galleryActions";
import { WGALLERY_ALBUM_CREATE_RESET } from "../../../actions/types";
...
const WAdminImages = ({ history }) => {
  ...
  const galleryWAlbumCreate = useSelector((state) => state.galleryWAlbumCreate);
  const { success: successCreate, error: successError } = galleryWAlbumCreate;
  ...
  useEffect(() => {
    if (successCreate) {
      ...
      dispatch({ type: WGALLERY_ALBUM_CREATE_RESET });
      setValidated(false);
    }
  ...
  const galleryHandler = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      dispatch(addWGalleryAlbums(name, images));
    }
    setValidated(true);
  };
